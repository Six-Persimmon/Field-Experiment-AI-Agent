...
#     def _choose_test(self, df, dv_col, iv_col=None, paired=False, known_pop=False):
#         """
#         Decide test based on classical flowchart:
#         - paired indicates within-subjects
#         - known_pop indicates known population parameters
#         """
#         # Categorical DV?
#         if pd.api.types.is_categorical_dtype(df[dv_col]) or df[dv_col].dtype == 'object':
#             # categorical DV: chi-square if IV exists
#             if iv_col:
#                 return 'Chi-Square Test'
#             else:
#                 return 'Descriptive Statistics'
...
#             if known_pop:
#                 return 'Z-test'
#             # reduce to means: check population mean assumption
#             # assume no => check variance equality
#             groups = df[iv_col].dropna().unique()
#             vtest = stats.levene(*(df[df[iv_col]==g][dv_col].dropna() for g in groups))
#             if vtest.pvalue < 0.05:
#                 return 'Welch t-test'
...
#         # More than 2 groups
#         if iv_col and df[iv_col].nunique() > 2:
#             # can reduce to means => ANOVA
#             return 'ANOVA'

#         # No IV or non-normal? fall back to nonparametric
#         # medians: Mann-Whitney if 2 groups
#         if iv_col and df[iv_col].nunique() == 2:
#             return 'Mann-Whitney U test'
#         if iv_col and df[iv_col].nunique() > 2:
#             return 'Kruskal-Wallis test'

#         # distributions: KS test for comparing distributions
#         if iv_col and df[iv_col].nunique() == 2:
#             return 'Kolmogorov-Smirnov test'
...
#     def _run_real_analysis(self, df):
#         # Automatically infer DV and IV
#         dv = df.columns[0]
#         iv = df.columns[1] if len(df.columns) > 1 else None
#         # Here you could set paired or known_pop flags based on design
#         test = self._choose_test(df, dv, iv)
#         # Execute
#         if test == 'Descriptive Statistics':
#             desc = df.describe().T[['mean', 'std']]
#             lines = [f"{col}: M={row['mean']:.2f}, SD={row['std']:.2f}" for col,row in desc.iterrows()]
#             return test, '; '.join(lines)

#         if test == 'Chi-Square Test':
#             ct = pd.crosstab(df[dv], df[iv])
#             chi2, p, dof, _ = stats.chi2_contingency(ct)
#             return test, f"χ²({dof})={chi2:.2f}, p={p:.3f}"

#         if test == 'Independent samples t-test':
#             g = df[iv].unique()
#             t, p = stats.ttest_ind(df[df[iv]==g[0]][dv].dropna(), df[df[iv]==g[1]][dv].dropna(), nan_policy='omit')
#             return test, f"t({len(df)-2})={t:.2f}, p={p:.3f}"

#         if test == 'Welch t-test':
#             g = df[iv].unique()
#             t, p = stats.ttest_ind(df[df[iv]==g[0]][dv].dropna(), df[df[iv]==g[1]][dv].dropna(), equal_var=False, nan_policy='omit')
#             return test, f"t={t:.2f}, p={p:.3f}"

#         if test == 'Z-test':
#             g = df[iv].unique()
#             z, p = ztest(df[df[iv]==g[0]][dv].dropna(), df[df[iv]==g[1]][dv].dropna())
#             return test, f"z={z:.2f}, p={p:.3f}"

#         if test == 'Paired samples t-test':
#             a = df.loc[df[iv]==df[iv].unique()[0], dv].dropna()
#             b = df.loc[df[iv]==df[iv].unique()[1], dv].dropna()
#             t, p = stats.ttest_rel(a, b)
#             return test, f"t={t:.2f}, p={p:.3f}"

#         if test == 'ANOVA':
#             groups = [df[df[iv]==g][dv].dropna() for g in df[iv].unique()]
#             fstat, p = stats.f_oneway(*groups)
#             return test, f"F={fstat:.2f}, p={p:.3f}"

#         if test == 'Mann-Whitney U test':
#             g = df[iv].unique()
#             u, p = stats.mannwhitneyu(df[df[iv]==g[0]][dv].dropna(), df[df[iv]==g[1]][dv].dropna(), alternative='two-sided')
#             return test, f"U={u:.2f}, p={p:.3f}"

#         if test == 'Kruskal-Wallis test':
#             groups = [df[df[iv]==g][dv].dropna() for g in df[iv].unique()]
#             h, p = stats.kruskal(*groups)
#             return test, f"H={h:.2f}, p={p:.3f}"

#         if test == 'Kolmogorov-Smirnov test':
#             g = df[iv].unique()
#             ks, p = stats.ks_2samp(df[df[iv]==g[0]][dv].dropna(), df[df[iv]==g[1]][dv].dropna())
#             return test, f"KS={ks:.2f}, p={p:.3f}"

#         return '[ERROR]', 'Unsupported test'

#     def _build_prompt_with_results(self, survey_design_text, preview, test, result):
#         parts = []
#         for ex in self.references:
#             parts.append(
#                 f"### Example Survey Design\n{ex['survey_design']}\n"
...
#             f"### Dataset Preview\n{preview}\n"
#             f"### Selected Test: {test}\n"
#             f"### Statistical Output: {result}\n"
#             "### Task Instruction: Generate an academic, rigorous analysis paragraph."
#             " Mirror the reference narrative structure and style, and report the test and statistics precisely."
#         )
#         return "\n---\n".join(parts)
...
#         df = pd.read_csv(dataset_path)
#         preview = self._format_dataset_preview(df)
#         test, result = self._run_real_analysis(df)
#         prompt = self._build_prompt_with_results(survey_design, preview, test, result)
#         resp = self.client.chat.completions.create(
#             model="gpt-4o",
#             messages=[
#                 {"role": "system", "content": "You are a PhD-level survey methods expert."},
